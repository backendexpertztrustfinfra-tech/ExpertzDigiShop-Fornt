...
// 🟢 NEW LOGIC: Helper to normalize data structure
const normalizeProductForCheckout = (p) => {
  if (!p) return null
  const hasVariants = p.variants && p.variants.length > 0
  
  // Convert images object {primary, front, etc} to array
  ...
  return {
    ...p,
    // Pick price from variant OR pricing object OR fallback
    price: hasVariants 
...
export default function CheckoutPage() {
  ...
  // 🟢 LOGIC FIX: Normalized buyNowProduct uses direct .price and .images[0]
  const items =
      ...
      ? [
          {
            ...
            price: buyNowProduct.price, // Now available via normalization
          },
        ]
      : cartItems
  ...
  const calculateBuyNowTotals = () => {
    ...
    // 🟢 LOGIC FIX: Using normalized price
    const subtotal = (buyNowProduct.price || 0) * buyNowQuantity
      ...
      const fetchProduct = async () => {
        try {
          ...
          if (response.success && response.product) {
            // 🟢 LOGIC FIX: Normalize data immediately after fetch
            setBuyNowProduct(normalizeProductForCheckout(response.product))
          } else {
  ...
  const handleRazorpayPayment = async (orderCreationResponse) => {
    ...
    const options = {
      ...
      prefill: userDetails,
      modal: { onclose: () => setIsProcessing(false) },
      theme: { color: "#FF4E50" },
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    // 🟢 LOGIC FIX: Use normalized item fields
    ...
    if (paymentMethod === "cod") {
      try {
        const response = await orderAPI.createOrder({ ...finalOrderData, paymentMethod: "cod" }, userToken)
      ...
      try {
        const response = await paymentAPI.createOrderForPayment({ amount: total, orderData: finalOrderData }, userToken)
  ...
  const getImageUrl = (imagePath) => {
    ...
    if (path.startsWith("http")) {
      if (path.includes("localhost:5000")) return path.replace("http://localhost:5000", BACKEND_URL);
      return path;
    }
  ...
  if (isLoading || cartLoading || buyNowLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white">
        ...
        <p className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] text-[#E75480]">Verifying...</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] text-[#333] selection:bg-[#FF4E50] selection:text-white">
      {/* UI IS UNCHANGED - ONLY LOGIC MAPPING FIXED BELOW */}
                ...
                <CardContent className="p-8 space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      ...
                      <Input className="h-12 rounded-xl border-[#FAD0C4]/50 font-bold" value={shippingInfo.firstName} onChange={(e) => handleInputChange("firstName", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Last Name</Label>
                      <Input className="h-12 rounded-xl border-[#FAD0C4]/50 font-bold" value={shippingInfo.lastName} onChange={(e) => handleInputChange("lastName", e.target.value)} />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1"> Email</Label>
                      <Input className="h-12 bg-[#FFF9F0] text-zinc-400 rounded-xl font-bold" value={shippingInfo.email} disabled />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Phone Node *</Label>
                      <Input className="h-12 rounded-xl border-[#FAD0C4]/50 font-bold" value={shippingInfo.phone} onChange={(e) => handleInputChange("phone", e.target.value)} />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Shipping Coordinates *</Label>
                    <Textarea className="border-[#FAD0C4]/50 rounded-xl min-h-[100px] font-bold" value={shippingInfo.street} onChange={(e) => handleInputChange("street", e.target.value)} placeholder="House No., Building, Area..." />
                  </div>
                  ...
                  <Button onClick={handleStep1Submit} className="w-full h-14 bg-[#FF4E50] text-white font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl">Select Funding Method</Button>
                </CardContent>
              </Card>
            )}
              ...
              <Card className="border-none shadow-xl rounded-[2rem] overflow-hidden bg-white">
                ...
                <CardContent className="p-8">
                  <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-5">
                    <Label htmlFor="razorpay" className={`flex items-center justify-between p-6 border rounded-[1.5rem] cursor-pointer ${paymentMethod === 'razorpay' ? 'border-[#FF4E50] bg-[#FFF5F7]' : 'border-zinc-100'}`}>
                      <div className="flex items-center gap-4">
                        <RadioGroupItem value="razorpay" id="razorpay" />
                        <div>
                          <p className="font-black text-sm uppercase">Pay Securely Online</p>
                          <p className="text-[10px] font-bold text-[#E75480] uppercase tracking-widest">Instant verification</p>
                        </div>
                      </div>
                      <CreditCard size={24} className={paymentMethod === 'razorpay' ? "text-[#FF4E50]" : "text-zinc-200"} />
                    </Label>
                    <Label htmlFor="cod" className={`flex items-center justify-between p-6 border rounded-[1.5rem] cursor-pointer ${paymentMethod === 'cod' ? 'border-[#FFB800] bg-[#FFF9F0]' : 'border-zinc-100'}`}>
                      <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-4 mt-10">
                    <Button variant="outline" onClick={() => setStep(1)} className="flex-1 h-14 rounded-2xl font-black uppercase text-[10px]">Back</Button>
                    <Button onClick={() => setStep(3)} className="flex-[2] h-14 bg-[#FF4E50] text-white font-black uppercase tracking-[0.2em] rounded-2xl">Final Review</Button>
                  </div>
                </CardContent>
              </Card>
            )}
              ...
              <Card className="border-none shadow-xl rounded-[2rem] overflow-hidden bg-white">
                ...
                <CardContent className="p-8 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <div className="p-6 bg-[#FFF5F7]/50 rounded-2xl border border-[#FAD0C4]/30 shadow-inner">
                        <p className="font-black text-sm uppercase text-[#333]">{shippingInfo.firstName} {shippingInfo.lastName}</p>
                        <p className="text-xs text-zinc-500 font-bold">{shippingInfo.street}, {shippingInfo.city}</p>
                        <p className="text-xs text-[#E75480] font-black uppercase tracking-widest">{shippingInfo.state} — {shippingInfo.zipCode}</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                  ...
                  <div className="flex gap-4 pt-6 border-t border-[#FAD0C4]/20">
                    <Button variant="outline" onClick={() => setStep(2)} className="flex-1 h-16 rounded-2xl font-black uppercase text-[10px] tracking-widest text-[#E75480]">Back</Button>
                    <Button onClick={handlePlaceOrder} disabled={isProcessing} className="flex-[2] h-16 bg-zinc-950 text-white rounded-2xl font-black uppercase tracking-[0.2em] text-xs">
                  ...
                  {items.map((item, idx) => (
                    <div key={idx} className="flex gap-5 group">
                      <div className="h-16 w-16 rounded-2xl bg-[#FFF5F7] border border-[#FAD0C4]/30 overflow-hidden flex-shrink-0 p-2 shadow-inner group-hover:scale-105 transition-transform">
                        {/* 🟢 LOGIC FIX: Normalized images[0] works perfectly here */}
                        <img 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex justify-between items-center mt-1.5">
                          ...
                          <p className="text-xs font-black text-[#333] italic tracking-tighter">{formatPrice((item.price) * item.quantity)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <style>
        {`
          ...
          .custom-scrollbar::-webkit-scrollbar-thumb { background-color: #FAD0C4; border-radius: 10px; }
        `}
      </style>
    </div>
  )
