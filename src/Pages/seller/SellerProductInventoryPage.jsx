...
import { Package, Plus, Eye, Edit, MoreHorizontal, Filter, Search, Download } from 'lucide-react'
...
// FIXED: Connect to live Render URLs
...
// FIXED: Robust Image Resolution Logic
const getProductImage = (product) => {
    if (product.images?.length > 0) {
        let imagePath = product.images[0]
        
        // Agar DB mein localhost hardcoded hai, toh use remove karein
        if (imagePath.includes("localhost:5000")) {
            imagePath = imagePath.split("/uploads/")[1]
            return `${UPLOAD_BASE_URL}/uploads/${imagePath.replace(/\\/g, "/")}`
        }

        // Agar full HTTP link hai (like Cloudinary), toh wahi return karein
        if (imagePath.startsWith("http")) return imagePath

        // Baki relative paths ke liye path clean karke Render URL jodein
...
export default function SellerProductInventoryPage() {
    ...
    const handleViewDetails = useCallback((productId) => { navigate(`/seller/product-details/${productId}`) }, [navigate])
    const handleEditProduct = useCallback((productId) => { navigate(`/seller/edit-product/${productId}`) }, [navigate])
    ...
    const handleExportCSV = () => {
      ...
      exportToCSV(
        ...
        ['SKU', 'Product', 'Category', 'Price', 'Stock', 'Status', 'Brand', 'CreatedDate']
      )
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-6 border-b pb-4">
                    <h1 className="text-3xl font-bold flex items-center gap-3">
                        <Package className="h-6 w-6 text-primary" /> Product Inventory ({filteredProducts.length} of {products.length})
                    </h1>
                ...
                <Card className="shadow-sm">
                    <CardHeader>
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <CardTitle className="text-xl font-semibold">Inventory List</CardTitle>
                    ...
                    <CardContent>
                        {products.length === 0 ? (
                            <div className="text-center py-20 border-2 border-dashed rounded-lg">
                                ...
                                <p className="text-muted-foreground mb-4 font-medium">Your inventory is empty.</p>
                                <Link to="/seller/add-product">
                                    ...
                                    <TableBody>
                                        {filteredProducts.map((product) => ( 
                                            <TableRow key={product._id} className="hover:bg-muted/30 transition-colors">
                                                <TableCell>
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="min-w-0">
                                                            ...
                                                            <div className="text-[11px] text-slate-500 font-mono mt-0.5">SKU: {product.sku || 'N/A'}</div>
                                                        </div>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="text-sm font-medium text-slate-600">{product.category}</TableCell>
                                                ...
                                                <TableCell>
                                                    <Badge variant="outline" className={`${getStatusColor(product.isActive ? "active" : "inactive")} border font-bold capitalize`}>
                                                ...
                                                <TableCell className="text-right">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end" className="w-40">
                                                            <DropdownMenuItem onClick={() => handleViewDetails(product._id)} className="cursor-pointer">
                                                                <Eye className="h-4 w-4 mr-2 text-blue-500" /> View Details
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => handleEditProduct(product._id)} className="cursor-pointer">
                                                                <Edit className="h-4 w-4 mr-2 text-amber-500" /> Edit Product
                                                            </DropdownMenuItem>
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
                                                </TableCell>
